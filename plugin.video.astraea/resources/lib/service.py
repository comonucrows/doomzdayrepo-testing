...
pause_services_prop = 'fenlightfork.pause_services'
firstrun_update_prop = 'fenlightfork.firstrun_update'
current_skin_prop = 'fenlightfork.current_skin'
...
class SetAddonConstants:
	def run(self):
		...
		import random
		addon_items = [
			('fenlightfork.playback_key', str(random.randint(1000, 10000))),
			('fenlightfork.addon_version', kodi_utils.addon_info('version')),
			('fenlightfork.addon_path', kodi_utils.addon_info('path')),
			('fenlightfork.addon_profile', kodi_utils.translate_path(kodi_utils.addon_info('profile'))),
			('fenlightfork.addon_icon', kodi_utils.translate_path(kodi_utils.addon_info('icon'))),
			('fenlightfork.addon_icon_mini', os.path.join(kodi_utils.addon_info('path'), 'resources', 'media', 'addon_icons', 'minis',
			os.path.basename(kodi_utils.translate_path(kodi_utils.addon_info('icon'))))),
			('fenlightfork.addon_fanart', kodi_utils.addon_fanart())
					]
...
class OnUpdateChanges:
	def run(self):
		...
		try:
			for method in list(filter(lambda x: x[0] != 'run', inspect.getmembers(OnUpdateChanges, predicate=inspect.isfunction))):
				if not get_setting('fenlightfork.updatechecks.%s' % method[0], 'false') == 'true':
					method[1](self)
	...
	def refresh_addon_keys(self):
		...
		if sha256(get_setting('fenlightfork.tmdb_api', '').encode('utf-8')).hexdigest() in tmdb_default_hashes:
			...
			show_dialog = True
		if sha256(get_setting('fenlightfork.trakt.client', '').encode('utf-8')).hexdigest() in trakt_default_hashes:
		...
		while not monitor.abortRequested():
			...
			try:
				...
				if status == 'failed': kodi_utils.logger('Fen Light', trakt_service_string % ('Failed. Error from Trakt', next_update_string))
					...
					if status == 'success' and get_setting('fenlightfork.trakt.refresh_widgets', 'false') == 'true':
...
class WidgetRefresher:
	def run(self):
		...
		while not monitor.abortRequested():
			try:
				wait_for_abort(10)
				offset = int(get_setting('fenlightfork.widget_refresh_timer', '60'))
	...
	def condition_check(self):
		...
		if kodi_utils.get_property('fenlightfork.window_loaded') == 'true': return True 
		try:
			window_stack = json.loads(kodi_utils.get_property('fenlightfork.window_stack'))
			if window_stack or window_stack == []: return True
		except: pass
		return False

	def set_next_refresh(self, _time):
		self.offset = int(get_setting('fenlightfork.widget_refresh_timer', '60'))
...
class AutoStart:
	def run(self):
		...
		from modules.settings import auto_start_fenlightfork
		if auto_start_fenlightfork(): kodi_utils.run_addon()
...
class AddonXMLCheck:
	def run(self):
		...
		self.addon_xml = kodi_utils.translate_path('special://home/addons/plugin.video.astraea/addon.xml')
	...
	def check_property(self, setting, tag_name):
		current_addon_setting = get_setting('fenlightfork.%s' % setting, None)
	...
	def reassign_addon_icon(self):
		...
		addon_icon_choice({'set_icon': get_setting('addon_icon_choice_name', 'fenlightfork_icon_01.png')})
