...
def addon_info(info):
	return xbmcaddon.Addon('plugin.video.astraea').getAddonInfo(info)

def addon_version():
	return get_property('fenlightfork.addon_version') or addon_info('version')

def addon_path():
	return get_property('fenlightfork.addon_path') or addon_info('path')

def addon_profile():
	return get_property('fenlightfork.addon_profile') or translate_path(addon_info('profile'))

def addon_icon():
	return get_property('fenlightfork.addon_icon') or translate_path(addon_info('icon'))

def addon_icon_mini():
	return get_property('fenlightfork.addon_icon_mini') or os.path.join(addon_info('path'), 'resources', 'media', 'addon_icons', 'minis',
														os.path.basename(translate_path(addon_info('icon'))))

def addon_fanart():
	return get_property('fenlightfork.addon_fanart') or translate_path(addon_info('fanart'))
...
def get_icon(image_name, image_folder='icons', image_type='png'):
	return 'https://raw.githubusercontent.com/%s/%s/main/packages/media/%s/%s.%s' \
			% (get_property('fenlightfork.update.username'), get_property('fenlightfork.update.location'), image_folder, image_name, image_type)

def get_addon_fanart():
	return get_property('fenlightfork.default_addon_fanart') or addon_fanart()

def build_url(url_params):
	return 'plugin://plugin.video.astraea/?%s' % urlencode(url_params)
...
def set_view_mode(view_type, content='files', is_external=None):
	if not get_property('fenlightfork.use_viewtypes') == 'true': return
	if is_external == None: is_external = external()
	if is_external: return
	view_id = get_property('fenlightfork.%s' % view_type) or None
...
def addon(addon_id='plugin.video.astraea'):
...
def run_addon(addon='plugin.video.astraea', block=False):
	return execute_builtin('RunAddon(%s)' % addon, block)

def external():
	return 'fenlightfork' not in get_infolabel('Container.PluginName')
...
def refresh_widgets():
	...
	kodi_refresh()
	if get_setting('fenlightfork.widget_refresh_notification', 'true') == 'true': notification('Widgets Refreshed', 2500)
...
def disable_enable_addon(addon_name='plugin.video.astraea'):
	import json
	try:
...
def update_kodi_addons_db(addon_name='plugin.video.astraea'):
	import time
...
def external_scraper_settings():
	try:
		external = get_property('fenlightfork.external_scraper.module')
...
def volume_checker():
	# 0% == -60db, 100% == 0db
	try:
		if get_property('fenlightfork.playback.volumecheck_enabled') == 'false' or get_visibility('Player.Muted'): return
		from modules.utils import string_alphanum_to_num
		max_volume = min(int(get_property('fenlightfork.playback.volumecheck_percent') or '50'), 100)
...
def get_all_icons():
	...
	username, location = get_property('fenlightfork.update.username'), get_property('fenlightfork.update.location')
