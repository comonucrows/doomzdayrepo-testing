	...
	def get_random_episode(self, continual=False, first_run=True):
		try:
			...
			if continual:
				episode_list = []
				try:
					episode_history = json.loads(kodi_utils.get_property('fenlightfork.random_episode_history'))
					if tmdb_key in episode_history: episode_list = episode_history[tmdb_key]
					else: kodi_utils.set_property('fenlightfork.random_episode_history', '')
				except: kodi_utils.set_property('fenlightfork.random_episode_history', '')
				episodes_data = [i for i in episodes_data if not i in episode_list]
				if not episodes_data:
					kodi_utils.set_property('fenlightfork.random_episode_history', '')
			...
			if continual:
				...
				kodi_utils.set_property('fenlightfork.random_episode_history', json.dumps(episode_history))
