...
class Navigator:
	...
	def tools(self):
		...
		if get_setting('fenlightfork.external_scraper.module') not in ('empty_setting', ''):
		...
		if get_setting('fenlightfork.use_viewtypes', 'true') == 'true' and not get_setting('fenlightfork.manual_viewtypes', 'false') == 'true':
	...
	def changelog_utils(self):
		...
		fenlightfork_clogpath = k.translate_path('special://home/addons/plugin.video.astraea/resources/text/changelog.txt')
		self.add({'mode': 'show_text', 'heading': 'Changelog', 'file': fenlightfork_clogpath, 'font_size': 'large', 'isFolder': 'false'}, 'Changelog', 'lists')
	...
	def build_shortcut_folder_contents(self):
		...
		if contents:
			for count, item in enumerate(contents):
				...
				icon = iconImage
				if iconImage:
					if iconImage.startswith('http') or 'plugin.video.astraea' in iconImage: original_image = True
	...
	def exit_media_menu(self):
		params = k.get_property('fenlightfork.exit_params')
		if params: return k.container_refresh_input(params)

	def tips(self):
		tips_location = 'special://home/addons/plugin.video.astraea/resources/text/tips'
