...
def get_personal_lists(params):
	...
	try:
		data = get_all_personal_lists(get_setting('fenlightfork.personal_list.list_sort', '0'))
		if data:
			if shuffle_lists:
				returning_to_list = 'build_personal_lists_contents' in kodi_utils.folder_path()
				if returning_to_list:
					try: data = json.loads(kodi_utils.get_property('fenlightfork.personal.lists.order'))
					except: pass
				else:
					shuffle(data)
					kodi_utils.set_property('fenlightfork.personal.lists.order', json.dumps(data))
			else:
				kodi_utils.clear_property('fenlightfork.personal.lists.order')
...
def build_personal_list(params):
	...
	try:
		...
		if page_no == 1 and not is_external: kodi_utils.set_property('fenlightfork.exit_params', kodi_utils.folder_path())
...
def make_new_personal_list(params):
	...
	if not external_creation and not is_retry and kodi_utils.confirm_dialog(
		...
		if suggested_author in ('Collection', 'Watchlist'): suggested_author = get_setting('fenlightfork.trakt.user')
