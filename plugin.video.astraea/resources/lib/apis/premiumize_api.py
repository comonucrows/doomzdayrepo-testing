...
DEFAULT_PREMIUMIZE_CLIENT_ID = '888228107'

class PremiumizeAPI:
	def __init__(self):
		self.token = get_setting('fenlightfork.pm.token', 'empty_setting')

	def auth(self):
		self.token = ''
		client_id = get_setting('fenlightfork.pm.client_id', 'empty_setting')
		if client_id in ('empty_setting', ''): client_id = DEFAULT_PREMIUMIZE_CLIENT_ID
		line = '%s[CR]%s[CR]%s'
