...
class TorBoxAPI:
	def __init__(self):
		self.token = get_setting('fenlightfork.tb.token')
	...
	def auth(self):
		self.token = ''
		try:
			url = 'https://api.torbox.app/v1/api/user/auth/device/start'
			response = session.get(url, params={'app': 'FenLightAM'}, timeout=20).json()
			data = response.get('data', response)
			device_code = data['device_code']
			user_code = data.get('code') or data.get('user_code')
			auth_url = data.get('verification_url') or data.get('friendly_verification_url')
			short_url = data.get('friendly_verification_url') or auth_url
			expires_in = int(data.get('expires_in') or data.get('expires') or 600)
			sleep_interval = max(1, int(data.get('interval') or 5))
		except:
			return notification('Unable to start TorBox authorization', 3000)
		qr_code = make_qrcode(auth_url) or ''
		copy2clip(auth_url)
		content = 'Please Scan the QR Code[CR]OR visit this URL: [B]%s[/B][CR]OR Enter this Code: [B]%s[/B]' % (short_url, user_code)
		progressDialog = progress_dialog('TorBox Authorize', qr_code)
		progressDialog.update(content, 0)
		poll_url = 'https://api.torbox.app/v1/api/user/auth/device/token'
		...
		while not progressDialog.iscanceled() and time_passed < expires_in and not self.token:
			...
			try:
				response = session.post(poll_url, json={'device_code': device_code}, timeout=20).json()
				data = response.get('data', response)
		...
		if self.token: ok_dialog(text='Success')
